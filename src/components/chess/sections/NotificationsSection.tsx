...
export const NotificationsSection = () => {
      ...
      const mockNotifications: Notification[] = [
        {
          ...
          message: 'Александр Петров хочет добавить вас в друзья',
          time: '5 минут назад',
          read: false,
        },
        {
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      case 'friend': return { name: 'UserPlus', color: 'text-blue-600 dark:text-blue-400' };
      case 'game': return { name: 'Play', color: 'text-green-600 dark:text-green-400' };
      case 'tournament': return { name: 'Trophy', color: 'text-yellow-600 dark:text-yellow-400' };
      case 'achievement': return { name: 'Award', color: 'text-purple-600 dark:text-purple-400' };
      default: return { name: 'Bell', color: 'text-gray-600 dark:text-gray-400' };
    }
  };
  ...
  const SettingToggle = ({ 
    ...
    <div className="flex items-center justify-between p-4 rounded-lg border bg-slate-50 dark:bg-slate-800/30 border-slate-200 dark:border-white/10">
      <div className="flex items-start gap-3 flex-1">
        <Icon name={icon} size={20} className="text-gray-600 dark:text-gray-400 mt-1" />
        <div>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2 text-gray-900 dark:text-white">
              <Icon name="Bell" className="text-blue-600 dark:text-blue-400" size={24} />
              Уведомления
              {unreadCount > 0 && (
                ...
                return (
                  ...
                  >
                    <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${
                      notification.read ? 'bg-slate-200 dark:bg-slate-700' : 'bg-blue-100 dark:bg-blue-800/50'
                    }`}>
                      <Icon name={iconData.name} className={iconData.color} size={20} />
                    </div>
      ...
      <Card className="bg-white dark:bg-slate-900/50 border-slate-200 dark:border-white/10">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-gray-900 dark:text-white">
            <Icon name="Settings" className="text-gray-600 dark:text-gray-400" size={24} />
