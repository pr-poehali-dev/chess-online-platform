...
export const MatchmakingSettingsModal = ({ settings, onSave, onClose }: Props) => {
  ...
  const sections = [
    ...
    },
    {
      ...
      content: (
        <p className="text-slate-300 text-sm leading-relaxed">
          Если после всех четырёх стадий живой соперник так и не нашёлся, игрок <span className="text-white font-semibold">автоматически и без уведомлений</span> переходит к игре с ботом. Среди ботов с рейтингом ±{ratingRange} от игрока выбирается случайный — так каждая партия будет с разным соперником. Если ни один бот не попадает в диапазон, берутся 5 ближайших по рейтингу и один выбирается случайно.
        </p>
      )
    },
  ];
